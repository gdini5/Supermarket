...
router.post('/', apiAuth, requireApiRole('client'), async (req, res, next) => {
  try {
    ...
    if (!cart || !cart.items || cart.items.length === 0)
      return res.status(400).json({ error: 'O carrinho está vazio.', code: 'EMPTY_CART' });

    const { deliveryMethod } = req.body;
    if (!deliveryMethod)
      return res.status(400).json({ error: 'O método de entrega é obrigatório.' });

    const sm = await Supermarket.findOne({ _id: cart.supermarketId, approved: true, active: true });
    if (!sm) return res.status(400).json({ error: 'Supermercado não encontrado ou não aprovado.' });

    // Validar método de entrega suportado pelo supermercado
    const validMethod = sm.deliveryMethods.find(d => d.type === deliveryMethod);
    if (!validMethod)
      return res.status(400).json({ error: 'Método de entrega não suportado por este supermercado.' });
...
router.get('/:id', apiAuth, async (req, res, next) => {
  try {
    ...
    const { role, userId } = req.apiUser;
    let hasAccess = role === 'admin' || order.clientId?._id.toString() === userId.toString();
    ...
    if (!hasAccess && role === 'courier') {
      hasAccess = order.courierId?._id?.toString() === userId.toString() ||
        (order.status === ORDER_STATUS.PREPARING && order.deliveryMethod === 'courier' && !order.courierId);
    }

    if (!hasAccess) return res.status(403).json({ error: 'Acesso negado a esta encomenda.' });
...
router.patch('/:id/status', apiAuth, async (req, res, next) => {
  try {
    ...
    if (!allowed[role]?.includes(status))
      return res.status(403).json({ error: 'Transição de estado não permitida para o seu perfil.' });
    ...
    if (role === 'supermarket') {
      ...
      if (!sm || sm._id.toString() !== order.supermarketId.toString())
        return res.status(403).json({ error: 'Esta encomenda não pertence ao seu supermercado.' });
    }

    // Courier não pode ter mais do que uma entrega ativa
    if (status === ORDER_STATUS.DELIVERING && role === 'courier') {
      const active = await Order.findOne({ courierId: userId, status: ORDER_STATUS.DELIVERING });
      if (active)
