...
export class OrderDetail implements OnInit, OnDestroy {
  ...
  /**
   * Cancelamento só está disponível durante os primeiros 5 minutos após a confirmação.
   * Desativar o botão no frontend evita erros desnecessários se a janela já fechou.
   */
  readonly canCancel = computed(() => {
    const o = this.order();
    if (!o || o.status !== 'pending') return false;
    
    const confirmedAt = new Date(o.confirmedAt).getTime();
  ...
  ngOnInit(): void {
    ...
    // Polling a cada 30 segundos para refletir mudanças de estado
