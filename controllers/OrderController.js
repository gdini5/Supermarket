...
OrderController.shop = async (req, res) => {
  try {
    ...
    const [supermarkets, categories] = await Promise.all([
      Supermarket.find({ approved: true, active: true }).select('name address'),
      Category.find({ active: true }).sort({ name: 1 })
    ]);
 
    res.render('orders/shop', {
      products: approvedProducts, supermarkets, categories,
      search: search || '', category: category || '',
      supermarketId: supermarketId || '', sort: sort || '',
...
OrderController.getCart = (req, res) => {
  ...
  res.render('orders/cart', { cart, error: req.query.error || null });
};
...
OrderController.addToCart = async (req, res) => {
  try {
    ...
    // FIX: comparar ambos como string para evitar mismatch ObjectId vs string
...
OrderController.checkout = async (req, res) => {
  try {
    ...
    console.error(err);
    res.render('error', { message: 'Erro ao criar encomenda.' });
  }
};
...
OrderController.list = async (req, res) => {
  try {
    ...
    if (role === 'client') {
      ...
      if (sm) filter.supermarketId = sm._id;
    } else if (role === 'courier') {
      // FIX: estafeta vê as encomendas em preparação COM entrega por courier
      // E também as suas próprias entregas
      filter = {
        $or: [
          { courierId: uid },
          { status: ORDER_STATUS.PREPARING, deliveryMethod: 'courier', courierId: null }
        ]
      };
    }
    // admin vê tudo
...
OrderController.show = async (req, res) => {
  try {
    ...
    // Courier pode ver encomendas em preparação disponíveis E as suas próprias
    if (!isOwner && role === 'courier') {
      isOwner = order.courierId?._id?.toString() === uid ||
        (order.status === ORDER_STATUS.PREPARING && order.deliveryMethod === 'courier' && !order.courierId);
    }
    ...
    console.error(err);
    res.render('error', { message: 'Erro ao carregar encomenda.' });
  }
};
...
OrderController.updateStatus = async (req, res) => {
  try {
    ...
    // FIX: courier também pode aceitar (delivering) encomendas que ainda não têm courier
    const allowed = {
    ...
    if (!allowed[role]?.includes(status))
      return res.render('error', { message: 'Transição de estado não permitida.' });
 
    // Verificar que o courier não tem outra entrega activa
    if (status === ORDER_STATUS.DELIVERING && role === 'courier') {
      const active = await Order.findOne({
        courierId: req.session.userId,
        status: ORDER_STATUS.DELIVERING
      });
      if (active)
        return res.render('error', { message: 'Já tens uma entrega em curso. Termina-a primeiro.' });
    }
    ...
    console.error(err);
    res.render('error', { message: 'Erro ao atualizar estado.' });
  }
};
